...
const FLUTTERWAVE_SERVICE_URL = process.env.FLUTTERWAVE_SERVICE_URL || 'http://localhost:5012';
...
export class FlutterwaveProvider implements PaymentProvider {
  ...
  private client = axios.create({
    baseURL: FLUTTERWAVE_SERVICE_URL,
    timeout: 30000,
    headers: {
  ...
  async createEscrow(params: EscrowCreateParams): Promise<{ providerEscrowId: string; status: string; metadata?: any }> {
    try {
      const response = await this.client.post('/escrow/create', {
        ...
        buyer_name: params.metadata?.buyerName,
      ...
      throw new Error(`Flutterwave escrow creation failed: ${error.response?.data?.error || error.message}`);
    }
  }
  ...
  async releaseEscrow(params: EscrowReleaseParams): Promise<{ transactionId: string; amount: number }> {
    try {
      ...
      throw new Error(`Flutterwave escrow release failed: ${error.response?.data?.error || error.message}`);
    }
  }
  ...
  async refundEscrow(params: EscrowRefundParams): Promise<{ transactionId: string; amount: number }> {
    try {
      ...
      throw new Error(`Flutterwave escrow refund failed: ${error.response?.data?.error || error.message}`);
    }
  }
  ...
  async getEscrowStatus(providerEscrowId: string): Promise<EscrowStatusResponse> {
    try {
      ...
      throw new Error(`Flutterwave status check failed: ${error.response?.data?.error || error.message}`);
    }
  }
