import { integer, serial, pgEnum, pgTable, text, timestamp, varchar, index, real } from "drizzle-orm/pg-core";

// ==================== ENUM DEFINITIONS ====================
// Extracted from inline enum declarations
...
// NOTE: recommendationPreferences and recommendationFeedback tables are now defined in schema-alerts-recommendations.ts
// They are re-exported at the end of this file to avoid duplication
...
export const shortletReviews = pgTable("shortletReviews", {
  ...
  photos: text("photos"), // JSON array of photo URLs (max 5)
...
// ==================== RE-EXPORTS FROM OTHER SCHEMA FILES ====================

// Re-export GNN-related tables
export {
  gnnPropertyNodes,
  gnnPropertyEdges,
  gnnValuationPredictions,
  gnnMarketTrendPredictions,
  gnnNeighborhoodIntelligence,
  gnnModelTrainingRuns,
  gnnModelPerformanceMetrics,
  gnnAlertSubscriptions,
  gnnAlertTriggers,
  type GNNPropertyNode,
  type InsertGNNPropertyNode,
  type GNNPropertyEdge,
  type InsertGNNPropertyEdge,
  type GNNValuationPrediction,
  type InsertGNNValuationPrediction,
  type GNNMarketTrendPrediction,
  type InsertGNNMarketTrendPrediction,
  type GNNNeighborhoodIntelligence,
  type InsertGNNNeighborhoodIntelligence,
  type GNNModelTrainingRun,
  type InsertGNNModelTrainingRun,
  type GNNModelPerformanceMetric,
  type InsertGNNModelPerformanceMetric,
  type GNNAlertSubscription,
  type InsertGNNAlertSubscription,
  type GNNAlertTrigger,
  type InsertGNNAlertTrigger,
} from "./gnn_schema";

// Re-export hybrid valuation tables
export {
  ...
  hybridValuations,
  dataQualityMetrics,
} from "./schema-hybrid-valuation";

// Re-export valuation tables
export {
  propertyValuations,
  valuationHistory,
  visualAssessments,
...
// Re-export valuation alert tables
export {
  valuationMonitoring,
  valuationChangeHistory,
  valuationAlertsSent,
  userAlertPreferences,
} from "./schema-valuation-alerts";

// Re-export valuation analytics tables
export {
  valuationViews,
  ...
  valuationABTests,
  valuationFeedback,
} from "./schema-valuation-analytics";

// Re-export monitoring tables
export {
  serviceHealth,
  apiUsage,
  metricsHourly,
...
// Note: mapAnalytics and emailDeliveryLogs are already defined in main schema.ts

// Re-export alert and recommendation tables
export {
  alertConfigurations,
  alertHistory,
  ...
  type InsertAlertAcknowledgment,
  type RecommendationPreference,
  type InsertRecommendationPreference,
  type RecommendationFeedback,
  type InsertRecommendationFeedback,
} from "./schema-alerts-recommendations";


// Investor Profiles
const riskToleranceEnum = pgEnum("riskTolerance", ["conservative", "moderate", "aggressive"]);
const investmentHorizonEnum = pgEnum("investmentHorizon", ["short", "medium", "long"]);

export const investorProfiles = pgTable("investor_profiles", {
  id: serial("id").primaryKey(),
  userId: integer("userId").notNull(), // FK to users
  minBudget: integer("minBudget").notNull(),
  maxBudget: integer("maxBudget").notNull(),
  minROI: integer("minROI").notNull(), // Stored as integer (8.5% = 85)
  riskTolerance: riskToleranceEnum("riskTolerance").notNull(),
  investmentHorizon: investmentHorizonEnum("investmentHorizon").notNull(),
  preferredCities: text("preferredCities"), // JSON array
  preferredNeighborhoods: text("preferredNeighborhoods"), // JSON array
  propertyTypes: text("propertyTypes"), // JSON array
  isActive: integer("isActive").default(1).notNull(),
...
// Saved Map Searches
export const savedMapSearches = pgTable("saved_map_searches", {
  id: serial("id").primaryKey(),
  userId: integer("userId").notNull(),
  name: varchar("name", { length: 255 }).notNull(),
  
  // Map bounds
  northEastLat: real("northEastLat").notNull(),
  northEastLng: real("northEastLng").notNull(),
  southWestLat: real("southWestLat").notNull(),
  southWestLng: real("southWestLng").notNull(),
  ...
  // Alert settings
  alertEnabled: integer("alertEnabled").default(1).notNull(),
...
export type SavedMapSearch = typeof savedMapSearches.$inferSelect;
export type InsertSavedMapSearch = typeof savedMapSearches.$inferInsert;

// Shortlet Availability Calendar
export const shortletAvailability = pgTable("shortlet_availability", {
  id: serial("id").primaryKey(),
  propertyId: integer("propertyId").notNull(),
  date: timestamp("date").notNull(),
  isAvailable: integer("isAvailable").default(1).notNull(),
  priceOverride: integer("priceOverride"), // Optional custom price for this date
  minimumStay: integer("minimumStay"), // Minimum nights for this date
  notes: text("notes"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
});

export type ShortletAvailability = typeof shortletAvailability.$inferSelect;
export type InsertShortletAvailability = typeof shortletAvailability.$inferInsert;

// Pricing Rules for Smart Pricing Dashboard
const pricingStrategyEnum = pgEnum("pricingStrategy", ["fixed", "dynamic", "market_based"]);
...
export const pricingRules = pgTable("pricing_rules", {
  ...
  weekendMultiplier: varchar("weekendMultiplier", { length: 10 }).default("1.2").notNull(),
  highSeasonStart: varchar("highSeasonStart", { length: 10 }), // MM-DD format
  highSeasonEnd: varchar("highSeasonEnd", { length: 10 }),
  highSeasonMultiplier: varchar("highSeasonMultiplier", { length: 10 }).default("1.5").notNull(),
  lowSeasonStart: varchar("lowSeasonStart", { length: 10 }),
  lowSeasonEnd: varchar("lowSeasonEnd", { length: 10 }),
  lowSeasonMultiplier: varchar("lowSeasonMultiplier", { length: 10 }).default("0.8").notNull(),
  enableDemandPricing: integer("enableDemandPricing").default(1).notNull(),
  demandMultiplierMin: varchar("demandMultiplierMin", { length: 10 }).default("0.8").notNull(),
  demandMultiplierMax: varchar("demandMultiplierMax", { length: 10 }).default("1.5").notNull(),
  lastMinuteDays: integer("lastMinuteDays").default(7).notNull(),
  lastMinuteDiscount: varchar("lastMinuteDiscount", { length: 10 }).default("0.15").notNull(),
  weeklyDiscount: varchar("weeklyDiscount", { length: 10 }).default("0.1").notNull(),
  monthlyDiscount: varchar("monthlyDiscount", { length: 10 }).default("0.2").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
});
...
// Special Event Pricing (e.g., holidays, festivals)
export const specialEventPricing = pgTable("special_event_pricing", {
  id: serial("id").primaryKey(),
  propertyId: integer("propertyId").notNull(),
  eventName: varchar("eventName", { length: 255 }).notNull(),
  startDate: timestamp("startDate").notNull(),
  endDate: timestamp("endDate").notNull(),
  priceMultiplier: varchar("priceMultiplier", { length: 10 }).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
});
...
// Custom Date Pricing (manual overrides for specific dates)
export const customDatePricing = pgTable("custom_date_pricing", {
  id: serial("id").primaryKey(),
  propertyId: integer("propertyId").notNull(),
  date: timestamp("date").notNull(),
  price: integer("price").notNull(),
...
// Market Pricing Recommendations (AI-generated suggestions)
export const marketPricingRecommendations = pgTable("market_pricing_recommendations", {
  id: serial("id").primaryKey(),
  propertyId: integer("propertyId").notNull(),
  recommendedBasePrice: integer("recommendedBasePrice").notNull(),
  confidence: varchar("confidence", { length: 10 }).notNull(), // 0.0 to 1.0
  competitorAvgPrice: integer("competitorAvgPrice"),
  marketDemandScore: varchar("marketDemandScore", { length: 10 }), // 0.0 to 1.0
  seasonalityFactor: varchar("seasonalityFactor", { length: 10 }),
...
// Availability Calendar (replaces shortletAvailability with pricing support)
export const availabilityCalendar = pgTable("availability_calendar", {
  id: serial("id").primaryKey(),
  propertyId: integer("propertyId").notNull(),
  date: timestamp("date").notNull(),
  isAvailable: integer("isAvailable").default(1).notNull(),
  price: varchar("price", { length: 20 }), // Calculated or custom price
...
// Saved Map Search Alerts (tracks new properties matching saved searches)
export const savedMapSearchAlerts = pgTable("saved_map_search_alerts", {
  id: serial("id").primaryKey(),
  savedSearchId: integer("savedSearchId").notNull(),
  propertyId: integer("propertyId").notNull(),
  userId: integer("userId").notNull(),
...
export type SavedMapSearchAlert = typeof savedMapSearchAlerts.$inferSelect;
export type InsertSavedMapSearchAlert = typeof savedMapSearchAlerts.$inferInsert;

// Competitor Tracking Jobs (tracks scraping job execution)
export const competitorTrackingJobs = pgTable("competitor_tracking_jobs", {
  id: serial("id").primaryKey(),
  jobType: varchar("jobType", { length: 50 }).notNull(), // 'scrape', 'analyze', 'notify'
  status: varchar("status", { length: 20 }).notNull().default("pending"), // 'pending', 'running', 'completed', 'failed'
  startedAt: timestamp("startedAt").defaultNow().notNull(),
  completedAt: timestamp("completedAt"),
  propertiesProcessed: integer("propertiesProcessed").default(0),
  emailsSent: integer("emailsSent").default(0),
  errorMessage: text("errorMessage"),
  metadata: text("metadata"), // JSON string for additional job data
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});

export type CompetitorTrackingJob = typeof competitorTrackingJobs.$inferSelect;
export type InsertCompetitorTrackingJob = typeof competitorTrackingJobs.$inferInsert;

// Competitor Listings (stores scraped competitor data)
export const competitorListings = pgTable("competitor_listings", {
  id: serial("id").primaryKey(),
  propertyId: integer("propertyId").notNull(), // Our property being tracked
  competitorUrl: varchar("competitorUrl", { length: 1000 }).notNull(),
  competitorPlatform: varchar("competitorPlatform", { length: 100 }).notNull(), // 'zillow', 'realtor', etc.
  competitorPrice: integer("competitorPrice"),
  competitorStatus: varchar("competitorStatus", { length: 50 }), // 'active', 'pending', 'sold'
  competitorListedDate: timestamp("competitorListedDate"),
  competitorUpdatedDate: timestamp("competitorUpdatedDate"),
  priceHistory: text("priceHistory"), // JSON array of price changes
...
export type InsertCompetitorListing = typeof competitorListings.$inferInsert;

// Property Tracking Preferences (per-property competitor tracking settings)
export const propertyTrackingPreferences = pgTable("property_tracking_preferences", {
  id: serial("id").primaryKey(),
  propertyId: integer("propertyId").notNull().unique(),
  trackingEnabled: integer("trackingEnabled").default(1).notNull(), // Boolean as integer
  notifyOnPriceChange: integer("notifyOnPriceChange").default(1).notNull(),
  notifyOnNewCompetitor: integer("notifyOnNewCompetitor").default(1).notNull(),
  notifyOnStatusChange: integer("notifyOnStatusChange").default(1).notNull(),
  checkFrequency: varchar("checkFrequency", { length: 20 }).default("daily").notNull(), // 'hourly', 'daily', 'weekly'
  priceChangeThreshold: integer("priceChangeThreshold").default(5), // Percentage threshold for notifications
  lastCheckedAt: timestamp("lastCheckedAt"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().notNull(),
});

export type PropertyTrackingPreference = typeof propertyTrackingPreferences.$inferSelect;
export type InsertPropertyTrackingPreference = typeof propertyTrackingPreferences.$inferInsert;

// ==================== JOB MONITORING ====================

export const jobMonitoring = pgTable("job_monitoring", {
  id: serial("id").primaryKey(),
  jobType: varchar("jobType", { length: 50 }).notNull(), // 'price_check', 'competitor_scan', 'market_summary'
  status: varchar("status", { length: 20 }).notNull().default("pending"), // 'pending', 'running', 'completed', 'failed', 'cancelled'
  propertyId: integer("propertyId"), // null for batch jobs
...
export const jobQueue = pgTable("job_queue", {
  ...
  payload: text("payload").notNull(), // JSON string for job parameters
...
export const jobExecutionMetrics = pgTable("job_execution_metrics", {
  ...
  itemsProcessed: integer("itemsProcessed").notNull().default(0),
  itemsFailed: integer("itemsFailed").notNull().default(0),
  averageItemTime: integer("averageItemTime"), // milliseconds per item
  peakMemoryUsage: integer("peakMemoryUsage"), // MB
...
// ==================== LAND RECORDS & C OF O VERIFICATION ====================
export * from "./schema-land-records";


// ==================== GNN RESULTS CACHING ====================
// Stores cached GNN inference results to improve performance and reduce computation costs

export const gnnValuationCache = pgTable("gnn_valuation_cache", {
  id: serial("id").primaryKey(),
  propertyId: integer("propertyId").notNull(),
  ...
  spatialFeatures: text("spatialFeatures"), // JSON: Neighborhood influence, centrality metrics
  temporalFeatures: text("temporalFeatures"), // JSON: Time-series features used
  explanation: text("explanation"), // JSON: Feature importance, SHAP values
  cachedAt: timestamp("cachedAt").defaultNow().notNull(),
  expiresAt: timestamp("expiresAt").notNull(), // Cache TTL
  isValid: integer("isValid").notNull().default(1), // 1=true, 0=false
}, (table) => ({
  propertyIdx: index("gnn_val_cache_property_idx").on(table.propertyId),
  expiresIdx: index("gnn_val_cache_expires_idx").on(table.expiresAt),
  validIdx: index("gnn_val_cache_valid_idx").on(table.isValid),
}));

export const gnnMarketTrendCache = pgTable("gnn_market_trend_cache", {
  id: serial("id").primaryKey(),
  neighborhoodId: varchar("neighborhoodId", { length: 100 }).notNull(),
  modelVersion: varchar("modelVersion", { length: 50 }).notNull(),
  trendDirection: varchar("trendDirection", { length: 20 }).notNull(), // up, down, stable
  trendStrength: real("trendStrength").notNull(), // -1 to 1
  priceChangePrediction: real("priceChangePrediction").notNull(), // % change
  timeHorizon: integer("timeHorizon").notNull(), // days
  confidence: real("confidence").notNull(),
  spatialDiffusion: text("spatialDiffusion"), // JSON: How trend spreads to nearby areas
  cachedAt: timestamp("cachedAt").defaultNow().notNull(),
  expiresAt: timestamp("expiresAt").notNull(),
  isValid: integer("isValid").notNull().default(1),
}, (table) => ({
  neighborhoodIdx: index("gnn_trend_cache_neighborhood_idx").on(table.neighborhoodId),
  expiresIdx: index("gnn_trend_cache_expires_idx").on(table.expiresAt),
}));

export const gnnNeighborhoodIntelCache = pgTable("gnn_neighborhood_intel_cache", {
  id: serial("id").primaryKey(),
  neighborhoodId: varchar("neighborhoodId", { length: 100 }).notNull(),
  modelVersion: varchar("modelVersion", { length: 50 }).notNull(),
  walkabilityScore: real("walkabilityScore"), // 0-100
  transitScore: real("transitScore"), // 0-100
  amenityDensity: real("amenityDensity"), // POIs per km²
  networkCentrality: real("networkCentrality"), // Graph centrality metric
  growthPotential: real("growthPotential"), // 0-100
  investmentScore: real("investmentScore"), // 0-100
  streetNetworkMetrics: text("streetNetworkMetrics"), // JSON: OSM-based metrics
  transitAccessibility: text("transitAccessibility"), // JSON: GTFS-based metrics
  cachedAt: timestamp("cachedAt").defaultNow().notNull(),
  expiresAt: timestamp("expiresAt").notNull(),
  isValid: integer("isValid").notNull().default(1),
}, (table) => ({
  neighborhoodIdx: index("gnn_intel_cache_neighborhood_idx").on(table.neighborhoodId),
  expiresIdx: index("gnn_intel_cache_expires_idx").on(table.expiresAt),
}));

export const gnnInferenceLog = pgTable("gnn_inference_log", {
  id: serial("id").primaryKey(),
  inferenceType: varchar("inferenceType", { length: 50 }).notNull(), // valuation, trend, neighborhood
  modelVersion: varchar("modelVersion", { length: 50 }).notNull(),
  ...
  cacheHit: integer("cacheHit").notNull().default(0), // 1=true, 0=false
  errorMessage: text("errorMessage"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({
  typeIdx: index("gnn_log_type_idx").on(table.inferenceType),
  createdIdx: index("gnn_log_created_idx").on(table.createdAt),
}));

export type GnnValuationCache = typeof gnnValuationCache.$inferSelect;
export type InsertGnnValuationCache = typeof gnnValuationCache.$inferInsert;
export type GnnMarketTrendCache = typeof gnnMarketTrendCache.$inferSelect;
export type InsertGnnMarketTrendCache = typeof gnnMarketTrendCache.$inferInsert;
export type GnnNeighborhoodIntelCache = typeof gnnNeighborhoodIntelCache.$inferSelect;
export type InsertGnnNeighborhoodIntelCache = typeof gnnNeighborhoodIntelCache.$inferInsert;
export type GnnInferenceLog = typeof gnnInferenceLog.$inferSelect;
export type InsertGnnInferenceLog = typeof gnnInferenceLog.$inferInsert;

// Re-export GNN alerts schema
export * from './schema-gnn-alerts';
